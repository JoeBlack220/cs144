...
void NetworkInterface::send_datagram(const InternetDatagram &dgram, const Address &next_hop) {
    ...
    if (_table.count(next_hop_ip) && _timer <= _table[next_hop_ip].ttl) {
        ...
        _retrans_arp_frame();
        // Waiting for the arp table updated then send the datagram
...
optional<InternetDatagram> NetworkInterface::recv_frame(const EthernetFrame &frame) {
    ...
    if (!_ethernet_address_equal(dst, ETHERNET_BROADCAST) &&
        !_ethernet_address_equal(dst, _ethernet_address)) {
        return {};
        ...
        if (msg.parse(frame.payload()) == ParseResult::NoError) {
            ...
            _table[ip].ttl = _timer + 30 * 1000;  

            // The ARP request is actually send the the current machine
...
void NetworkInterface::_retrans_arp_frame(){
